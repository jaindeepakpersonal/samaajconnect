...
/// and what its tests are actually about - the tenant query filter, the
/// eight-state workflow, and the outbox row landing in the same transaction as
/// the transition - are all database claims. The outbox guarantee is
/// transactional, so proving it needs a real database and not a real broker.
/// </remarks>
public sealed class SocialIssuesApiFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
    public const string SigningKey = "integration-test-signing-key-at-least-32-chars";

    private readonly PostgreSqlContainer _postgres = new PostgreSqlBuilder()
        .WithImage("postgres:16-alpine")
        .WithDatabase("samaajconnect_social_issues")
    ...
    public async Task ResetDatabaseAsync()
    {
        ...
        var dbContext = scope.ServiceProvider.GetRequiredService<SocialIssuesDbContext>();

        await dbContext.Database.ExecuteSqlRawAsync(
            "TRUNCATE TABLE issue_status_history, social_issues, outbox_messages "
            + "RESTART IDENTITY CASCADE;");
    }

    public async Task<T> WithDbContextAsync<T>(Func<SocialIssuesDbContext, Task<T>> action)
    {
        await using var scope = Services.CreateAsyncScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<SocialIssuesDbContext>();
