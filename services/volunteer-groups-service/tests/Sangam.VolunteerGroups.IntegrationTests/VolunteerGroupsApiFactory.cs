...
/// and what its tests are actually about - the tenant query filter, the join
/// flow, and the outbox row landing in the same transaction as the application
/// - are all database claims. The outbox guarantee is transactional, so proving
/// it needs a real database and not a real broker.
/// </remarks>
public sealed class VolunteerGroupsApiFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
    public const string SigningKey = "integration-test-signing-key-at-least-32-chars";

    private readonly PostgreSqlContainer _postgres = new PostgreSqlBuilder()
        .WithImage("postgres:16-alpine")
        .WithDatabase("samaajconnect_volunteer_groups")
    ...
    public async Task ResetDatabaseAsync()
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            "TRUNCATE TABLE group_members, group_applications, volunteer_groups, "
            + "outbox_messages RESTART IDENTITY CASCADE;");
    }

    public async Task<T> WithDbContextAsync<T>(Func<VolunteerGroupsDbContext, Task<T>> action)
    {
        await using var scope = Services.CreateAsyncScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<VolunteerGroupsDbContext>();
