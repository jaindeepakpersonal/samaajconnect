...
/// <summary>
/// Consumes every platform domain event and records it.
/// </summary>
/// <remarks>
    ...
    : BackgroundService
{
    ...
    private async Task ConsumeLoopAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.Subscribe(_options.TopicPattern);

        logger.LogInformation(
            "Audit consumer subscribed to {Pattern} as group {GroupId}",
            _options.TopicPattern,
            _options.GroupId);
        ...
        {
            consumer.Close();
            logger.LogInformation("Audit consumer stopped");
        }
    }
        ...
        for (var attempt = 1; attempt <= _options.MaxAttempts; attempt++)
        {
            try
            {
                ...
                var outcome = await sender.Send(
                    new CreateProfileForNewUserCommand(envelope), stoppingToken);
