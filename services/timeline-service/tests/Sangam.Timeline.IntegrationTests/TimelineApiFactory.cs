...
/// moderation lifecycle and the outbox row landing in the same transaction as
/// the post - are all database claims. The outbox guarantee is transactional,
/// so proving it needs a real database and not a real broker.
/// </remarks>
public sealed class TimelineApiFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
    public const string SigningKey = "integration-test-signing-key-at-least-32-chars";

    private readonly PostgreSqlContainer _postgres = new PostgreSqlBuilder()
        .WithImage("postgres:16-alpine")
        .WithDatabase("samaajconnect_timeline")
    ...
    public async Task ResetDatabaseAsync()
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            "TRUNCATE TABLE moderation_actions, post_reactions, post_comments, timeline_posts, "
            + "outbox_messages RESTART IDENTITY CASCADE;");
    }

    public async Task<T> WithDbContextAsync<T>(Func<TimelineDbContext, Task<T>> action)
    {
        await using var scope = Services.CreateAsyncScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<TimelineDbContext>();
